...
function Teacher(){
    ...
    const handleSave = async() => {
        try{
            ...
            if (!teacher.fullName || !teacher.birthday || !teacher.phone || !teacher.email || !teacher.address || !teacher.expertise || teacher.certificates.length === 0) {
    ...
    <div className='teacher'>
      ...
      <div className="teacherForm">
        ...
        <div className='groupInp'>
          ...
          <input type="text" name='fullName' value={teacher.fullName} onChange={(e) => setTeacher({...teacher, fullName: e.target.value})} />
        </div>

        <div className='groupInp'>
          <label htmlFor="msgv">MSGV: </label>
          <input type="text" name='msgv' value={teacher.msgv} onChange={(e) => setTeacher({...teacher, msgv: e.target.value})} />
        </div>

        <div className='groupInp'>
            <label htmlFor="birthday">Ngày sinh: </label>
            <input type="date" name='birthday' value={teacher.birthday.toISOString().substr(0,10)} onChange={(e) => setTeacher({...teacher, birthday: new Date(e.target.value)})} />
        </div>

        <div className='groupInp'>
            <label htmlFor="gender">Giới tính: </label> <br />
            Nam <input type="radio" name="gender" value="true" checked={teacher.gender === true} 
                onChange={() => setTeacher({ ...teacher, gender: true })}/>
            Nữ <input type="radio" name="gender" value="false" checked={teacher.gender === false}
        ...
        <div className='groupInp'>
            ...
            <input type="text" name='phone' value={teacher.phone} onChange={(e) => setTeacher({...teacher, phone: e.target.value})} />
        </div>

        <div className='groupInp'>
            <label htmlFor="email">Email: </label>
            <input type="text" name='email' value={teacher.email} onChange={(e) => setTeacher({...teacher, email: e.target.value})} />
        </div>

        <div className='groupInp'>
            <label htmlFor="address">Địa chỉ: </label>
            <input type="text" name='address' value={teacher.address} onChange={(e) => setTeacher({...teacher, address: e.target.value})} />
        </div>

        <div className='groupInp'>
            <label htmlFor="degree">Bằng cấp: </label>
            <input type="text" name='degree' value={teacher.degree} onChange={(e) => setTeacher({...teacher, degree: e.target.value})} />
            <button id='modifier' onClick={addCertificate}><i className="plus icon"></i></button> {/* Nút để thêm bằng cấp */}
        </div>
        ...
        <div className='groupInp'>
            ...
            <input type="text" name='expertise' value={teacher.expertise} onChange={(e) => setTeacher({...teacher, expertise: e.target.value})} />
        </div>
      </div>
